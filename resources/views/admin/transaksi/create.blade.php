...
<body class="font-body-md text-body-md overflow-x-hidden">
    ...
    <header class="ml-64 h-16 bg-surface-container-lowest border-b border-outline-variant flex justify-between items-center px-margin-desktop sticky top-0 z-40">
        ...
        <div class="flex items-center gap-6">
            <div class="flex items-center gap-3 pl-6 border-l border-outline-variant">
                ...
                <img alt="Profil Admin" class="w-10 h-10 rounded-full object-cover border border-outline-variant" src="https://ui-avatars.com/api/?name=Admin+Utama&background=random"/>
            </div>
        </div>
    </header>
    ...
    <main class="ml-64 p-margin-desktop min-h-[calc(100vh-4rem)]">
        ...
        @if(session('success'))
        <div class="mb-6 px-4 py-3 bg-secondary-container text-on-secondary-container rounded flex items-center gap-3 shadow-sm border border-secondary/20">
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-gutter mb-stack-lg">
            <div class="bg-surface-container-lowest p-6 border border-outline-variant rounded transition-all hover:shadow-lg hover:shadow-black/5 group">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <span class="text-secondary font-label-md">Total Data</span>
                </div>
                <h3 class="font-label-md text-label-md text-on-surface-variant mb-1">Total Transaksi</h3>
                <p class="font-headline-sm text-headline-sm text-primary">{{ $transaksis->total() }}</p>
            </div>
            <div class="bg-surface-container-lowest p-6 border border-outline-variant rounded transition-all hover:shadow-lg hover:shadow-black/5 group">
                <div class="flex justify-between items-start mb-4">
                    <span class="material-symbols-outlined p-2 bg-secondary-container text-secondary rounded" data-icon="payments">payments</span>
                </div>
                <h3 class="font-label-md text-label-md text-on-surface-variant mb-1">Pendapatan Terpantau</h3>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low border-b border-outline-variant">
                        ...
                        <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-widest">Pelanggan</th>
                ...
                <tbody class="divide-y divide-outline-variant">
                    ...
                    <tr class="table-row-hover transition-colors cursor-pointer" onclick="window.location='{{ route('admin.transaksi.edit', $transaksi->id_transaksi) }}'">
                        <td class="px-6 py-5 font-label-md text-primary">TRX-{{ str_pad($transaksi->id_transaksi, 4, '0', STR_PAD_LEFT) }}</td>
                        
                        <td class="px-6 py-5">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-secondary-container flex items-center justify-center text-[10px] text-on-secondary-container font-bold uppercase">
                                    {{ substr($transaksi->pelanggan->nama_pelanggan ?? 'XX', 0, 2) }}
                                </div>
                                <div class="flex flex-col">
                                    <span class="font-body-md font-semibold text-on-surface">{{ $transaksi->pelanggan->nama_pelanggan ?? 'Data Dihapus' }}</span>
                                    <span class="text-[12px] text-on-surface-variant">ID: {{ $transaksi->id_pelanggan }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 text-on-surface-variant">
                            <div class="flex items-center gap-2">
                                ...
                                {{ \Carbon\Carbon::parse($transaksi->tanggal_pembayaran)->translatedFormat('d M Y') }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-5 text-center">
                            <div class="flex justify-center gap-2" onclick="event.stopPropagation();">
                                <a href="{{ route('admin.transaksi.edit', $transaksi->id_transaksi) }}" class="p-2 hover:bg-surface-container rounded-full text-secondary transition-colors" title="Edit">
                                    <span class="material-symbols-outlined text-[20px]">edit</span>
                                </a>
                                
                                <form action="{{ route('admin.transaksi.destroy', $transaksi->id_transaksi) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus transaksi TRX-{{ str_pad($transaksi->id_transaksi, 4, '0', STR_PAD_LEFT) }} ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center text-on-surface-variant">
                                <span class="material-symbols-outlined text-[48px] mb-4 text-outline-variant">receipt_long</span>
                                <p class="font-body-md">Belum ada data transaksi yang dicatat.</p>
                                <a href="{{ route('admin.transaksi.create') }}" class="mt-4 text-primary font-semibold hover:underline">Tambah Transaksi Sekarang</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
