...
<head>
    ...
    <title>Update Pelanggan - Bangun Mulyo</title>
    ...
    <main class="ml-64 min-h-screen bg-surface">
        ...
        <header class="h-20 px-margin-desktop flex items-center justify-between border-b border-outline-variant bg-surface-container-lowest">
            <div>
                <nav class="flex text-on-surface-variant font-label-sm text-label-sm gap-2">
                    ...
                    <span class="text-primary font-bold">Update Pelanggan</span>
                </nav>
            </div>
        ...
        <section class="p-margin-desktop max-w-4xl mx-auto">
            <div class="mb-stack-lg">
                <h2 class="font-headline-md text-headline-md text-primary mb-2">Update Pelanggan</h2>
                <p class="text-on-surface-variant">Perbarui informasi pelanggan di bawah ini untuk memperbarui data dalam sistem Bangun Mulyo.</p>
            </div>

            <div class="luxury-card p-margin-desktop rounded-lg">
                
                <form action="{{ route('admin.pelanggan.update', $pelanggan->id_pelanggan) }}" method="POST" class="space-y-stack-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-gutter">
                        <div class="flex flex-col gap-2">
                            ...
                            <input class="form-input bg-surface-container text-on-surface-variant cursor-not-allowed font-mono" id="customer-id" readonly type="text" value="BM-{{ sprintf('%05d', $pelanggan->id_pelanggan) }}">
                            <span class="text-[11px] text-on-surface-variant italic">ID tidak dapat diubah.</span>
                        </div>
                        
                        <div class="flex flex-col gap-2">
                            <label class="font-label-md text-label-md uppercase tracking-wider text-primary" for="customer-name">Nama Pelanggan</label>
                            <input class="form-input @error('nama_pelanggan') input-error @enderror" name="nama_pelanggan" id="customer-name" placeholder="Masukkan nama lengkap..." required type="text" value="{{ old('nama_pelanggan', $pelanggan->nama_pelanggan) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-gutter">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                ...
                                <input class="form-input w-full pl-12 @error('no_telepon') input-error @enderror" name="no_telepon" id="phone-number" placeholder="812 3456 7890" required type="tel" value="{{ old('no_telepon', $pelanggan->no_telepon) }}">
                            </div>
                            @error('no_telepon')
                                <span class="text-[12px] text-error">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <textarea class="form-input resize-none @error('alamat') input-error @enderror" name="alamat" id="address" placeholder="Jl. Raya Bangun Mulyo No. 12..." required rows="4">{{ old('alamat', $pelanggan->alamat) }}</textarea>
                        @error('alamat')
                            <span class="text-[12px] text-error">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="pt-stack-lg border-t border-outline-variant flex flex-col md:flex-row justify-end gap-gutter mt-stack-lg">
                        ...
                        <a href="{{ route('admin.pelanggan.index') }}" class="px-8 py-3 border border-secondary text-secondary font-label-md text-label-md uppercase tracking-widest hover:bg-secondary-container transition-colors duration-300 flex items-center justify-center">
                            Batal
                        </a>
                        
                        <button class="px-8 py-3 bg-primary text-on-primary font-label-md text-label-md uppercase tracking-widest hover:opacity-90 transition-opacity shadow-lg" type="submit" id="btn-submit">
                            Update Pelanggan
                        </button>
                    </div>
                </form>
            ...
            <div class="mt-stack-lg flex items-start gap-4 p-4 bg-surface-container-high rounded-sm">
                ...
                <p class="text-label-sm font-label-sm text-on-surface-variant">
                    <span class="font-bold text-primary">Informasi:</span> Data yang diperbarui akan langsung berdampak pada seluruh transaksi dan riwayat yang terhubung dengan pelanggan ini di masa mendatang.
                </p>
            </div>
        </section>
